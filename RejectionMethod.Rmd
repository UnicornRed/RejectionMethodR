...
epsilon <- 1e-6
gradient <- (log(dnorm(points_x + epsilon)) - log(dnorm(points_x - epsilon))) / (2 * epsilon)
...
plot_d_lower_upper(dnorm, points_x, points_y, gradient, a, b)
```
...
```{r}
adapt_samp <- r_adaptive_method_rejection(10000, function(n) runif(n, a, b), dnorm, function(x) dunif(x, min = a, max = b), a, b, M)
